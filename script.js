...
const API_BASE = "https://colten-unobviated-incisively.ngrok-free.app";
...
const pushNowPlaying = (() => {
  let lastNote = "", lastLyric = "", lastPitchComparison = null;
  return async (note, lyric, pitchComparison = null) => {
    note = (note || "").trim();
    lyric = (lyric || "").trim();
    if (note === lastNote && lyric === lastLyric && JSON.stringify(pitchComparison) === JSON.stringify(lastPitchComparison)) return; // dedupe
    lastNote = note; lastLyric = lyric; lastPitchComparison = pitchComparison;
    console.debug("[nowplaying->API]", { note, lyric, pitchComparison });
    try {
      const res = await fetch(`${API_BASE}/nowplaying`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ note, lyric, pitchComparison }),
        keepalive: true,
      });
...
document.getElementById('playBtn').addEventListener('click', async () => {
    ...
    pushNowPlaying(lead, currentLyricText, null); // No pitch comparison data for regular updates
    }, 0.016);
